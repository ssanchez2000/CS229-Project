...
def all_train(loader_train, gender_model,smile_model, loss_fn, gender_optimizer,smile_optimizer, dtype,num_epochs=1, print_every=20):
    """
...
def validate(gender_model,smile_model, loader, dtype):
    """
...
def pca_model(x_var,nComp,dtype):
    ...
    for indx in range(img.shape[0]):
        ...
        G = img[indx,1,:,:]
        B = img[indx,2,:,:]
        mu_r = npnp.mean(R, axis=0)
        mu_g = npnp.mean(G, axis=0)
        mu_b = npnp.mean(B, axis=0)

        pca = sklearn.decomposition.PCA()

        pca.fit(R)
        Rhat = npnp.dot(pca.transform(R)[:, :nComp], pca.components_[:nComp, :])
        Rhat += mu_r

        pca.fit(G)
        Ghat = np.dot(pca.transform(G)[:, :nComp], pca.components_[:nComp, :])
        Ghat += mu_g

        pca.fit(B)
        Bhat = npnp.dot(pca.transform(B)[:, :nComp], pca.components_[:nComp, :])
        Bhat += mu_b
        img[indx,0, :, :], img[indx,1, :, :], img[indx,2, :, :] = Rhat, Ghat, Bhat
...
acc_all,acc_gender,acc_smile=validate(gender_model,smile_model, val_loader, dtype)
