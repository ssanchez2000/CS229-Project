...
def all_train(loader_train, all_model,gender_model,smile_model, loss_fn, all_optimizer,gender_optimizer,smile_optimizer dtype,num_epochs=1, print_every=10):
    ...
    for i in range(num_epochs):
        for t, (x,y,z) in enumerate(loader_train):
            ...
            scores_gender=gender_model(x_var)
            scores_smile=smile_model(x_var)
...
gender_optimizer = optim.Adam(model.parameters(), lr=1e-4,weight_decay=2e-3)
smile_optimizer = optim.Adam(model.parameters(), lr=1e-4,weight_decay=0.010)

print("start training")
loss_all_history, loss_gender_history,loss_smile_history, acc_all_history, acc_gender_history,acc_smile_history=all_train(train_loader, all_model,gender_model,smile_model, loss_fn, all_optimizer,gender_optimizer, smile_optimizer dtype,num_epochs=15, print_every=20)
